...
type SceneInput={label?:string;caption?:string;prophetPresent?:boolean};
...
export async function POST(request:NextRequest){
 try{
  ...
  for(const scene of scenes){
   ...
   const prompt=["Create a cinematic photorealistic live-action historical documentary still, not an illustration, not animation, not 2D art, not a cartoon, not CGI.","Natural skin, fabric, stone, dust, fire, water and atmospheric lighting; realistic camera optics and film color grading; 16:9 widescreen composition; no text, captions, logos or watermarks.","Respectful Islamic visual treatment. No divine beings, angels, halos, glowing faces, icons, or religious caricatures.",sacredRule,`Story context: ${title}. Scene: ${String(scene.label||"").slice(0,100)}. Event: ${String(scene.caption||"").slice(0,700)}`].join(" ");
